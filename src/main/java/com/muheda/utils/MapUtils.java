...
//        System.out.println(new BigDecimal(String.valueOf(bd_lon)).floatValue()+","+new BigDecimal(String.valueOf(bd_lat)).floatValue());
    ...
    //计算出一段路程的最小矩形
    public static RectDouble minimumRectangle(List<LngAndLat> list) {
        ...
        //最小包围矩形的最小点(逻辑上的）
        rectDouble.setLbX(lngs.get(0));
        rectDouble.setLbY(lats.get(0));

        //最小包围矩形的最大的点（逻辑上的）
        rectDouble.setRtX(lngs.get(lngs.size() - 1));
        rectDouble.setRtY(lats.get(lats.size() - 1));
